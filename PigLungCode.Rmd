...
ps.Env = subset_samples(ps.rncp, sample_data(ps.rncp)$Sample == "Environment") 
...
permanova = adonis(unifrac.dist ~ Farm*SampleType, data = Envmeta) #permanova shows significant differences in farm*SampleType but pairwise testing with p value correction indicates that between the two farms none of the same samples are significantly different

permanova

Envmeta=unite(Envmeta, FarmSample, c(Farm, SampleType), remove = F)

pairwise.adonis(unifrac.dist, Envmeta$FarmSample)
...
set.seed(100)
anosim(unifrac.dist, Envmeta$SampleType, permutations = 9999) # R = 0.0732, significance = 0.056
...
anosim(unifrac.dist, Pigmeta$SampleType, permutations = 9999) #R = 0.39, significance = 1e-4
```
...
g1=ggplot(PD, aes(SampleType, PD, color = Farm)) + stat_boxplot(fill = NA) + geom_jitter(width = 0.2, alpha = 0.5)
...
g2=ggplot(Shannon, aes(SampleType, Shannon, color = Farm)) + stat_boxplot(fill = NA) + geom_jitter(width = 0.2, alpha = 0.5)+expand_limits(y=0)
...
g1=ggplot(PD, aes(SampleType, PD, color = Farm)) + stat_boxplot(fill = NA) + geom_jitter(width = 0.2, alpha = 0.5)+expand_limits(y=0)
...
```{r, what happens when we remove air samples from SourceTracker analysis?}
...
```{r, aggregation of sourcetracker txt files into dataframe, not using this analysis}
