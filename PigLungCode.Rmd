...
```{r, make PCoA for all samples with unifrac}
...
g2=ggplot(Shannon, aes(SampleType, Shannon, color = Farm)) + stat_boxplot(fill = NA) + geom_jitter(width = 0.2, alpha = 0.5)
...
STDF$variable=factor(STDF$variable,levels=c("Food","Water", "Soil", "Unknown", "Air"))

STDF$Group=factor(STDF$Group,levels=c("lavage","A_swab", "B_swab", "B_tissue", "A_tissue"))

ggplot() + geom_bar(aes(y = value, x = Group, fill = variable), data = STDF, stat ="identity")+scale_fill_manual(values=c( "green4", "royalblue", "brown", "grey52", "deepskyblue3"))

STDF_all = bind_rows(swab_ST_all, lavage_ST_all, tissue_ST_all) %>%
  reshape2::melt(.)

STDF_all$variable=factor(STDF_all$variable,levels=c("Food","Water", "Soil", "Unknown", "Air"))

STDF_all$Group=factor(STDF_all$Group,levels=c("lavage","A_swab", "B_swab", "B_tissue", "A_tissue"))

ggplot(STDF_all, aes(Group, value)) + facet_grid(. ~ variable) + stat_boxplot(fill = NA) + geom_jitter(width = 0.2, alpha = 0.5)+scale_fill_manual(values=c( "green4", "royalblue", "brown", "grey52", "deepskyblue3"))
```
