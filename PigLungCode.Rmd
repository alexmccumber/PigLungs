...
uniques=getUniques(vegan_otu(ps.prok))
sequences=names(uniques)
uniquesToFasta(uniques,fout = "~/pigs.fasta", ids=sequences)
```
...
ps.tree = merge_phyloseq(ps.prok, pig.tree)
```
...
ps.r=readRDS("~/PigLungs/ps.r.RDS")
ps.rncp=subset_samples(ps.r, sample_data(ps.r)$Sample != "NTC"& sample_data(ps.tree)$SampleName != "WCF_AIR_BLANK_S92" & sample_data(ps.tree)$SampleName != "NCAT_AIR_BLANK_S73") %>%
...
plot_ordination(ps.rncp, ., type="samples", color = "Sample", shape = "Farm")+stat_ellipse()
...
permanova = adonis(unifrac.dist ~ Farm*SampleType, data = Envmeta) #permanova shows no significant differences (p=0.06) in farm*SampleType but there are significant differences in farm and sample type
...
set.seed(100)
anosim(unifrac.dist, Envmeta$SampleType, permutations = 9999) # R = 0.079, significance = 0.06

anosim(unifrac.dist, Envmeta$Farm, permutations = 9999) #R = 0.47, significance = 1e-4
...
anosim(unifrac.dist, Pigmeta$Farm, permutations = 9999) #R = 0.51, significance = 1 e-4

anosim(unifrac.dist, Pigmeta$SampleType, permutations = 9999) #R = 0.43, significance = 1e-4
```
...
g1=ggplot(PD, aes(SampleType, PD, color = Farm)) + stat_boxplot(fill = NA) 
...
g2=ggplot(Shannon, aes(SampleType, Shannon, color = Farm)) + stat_boxplot(fill = NA) +expand_limits(y=0)

grid.arrange(g1, g2, nrow = 1)

g=arrangeGrob(g1,g2,nrow=1)

ggsave(file="~/PigLungs/LungalphaDiv.png", g, width = 20, height = 10, units = "cm")

#comparison of alpha diversities at each farm for the pig smaples
...
pairwise.wilcox.test(PD$PD, PD$FarmSample, p.adjust.method = "BH") #only swab samples between farms are different

kruskal.test(PD ~ SampleType, data = PD)
pairwise.wilcox.test(PD$PD, PD$SampleType, p.adjust.method = "BH") #indicates Tissue samples are different from lavage and swab but lavage and swab are not different

fit = aov(Shannon ~ Farm*SampleType, data = Shannon)
plot(fit)
shapiro.test(Shannon$Shannon) # p=0.24 so data is normally distributed
...
g1=ggplot(PD, aes(SampleType, PD, color = Farm)) + stat_boxplot(fill = NA) + expand_limits(y=0)
...
ggsave(file="~/PigLungs/EnvalphaDiv.png", g, width = 20, height = 10, units = "cm")
...
kruskal.test(PD ~ FarmSample, data = PD) #p=0.24 so none of the medians are different between the Farm*SampleType

kruskal.test(PD ~ SampleType, data = PD) #p=0.177 so none of the medians are different between SampleTypes

pairwise.wilcox.test(PD$PD, PD$Farm, p.adjust.method = "BH") #p=0.075 so shows there is no difference between farms 
...
ps.pig=subset_samples(ps.rncp, sample_data(ps.rncp)$Sample=="Pig") %>%
  subset_samples(., sample_data(ps.rncp)$SampleType == "Lavage" | sample_data(ps.rncp)$SampleType == "Swab") %>%
  prune_species(speciesSums(.) >0, .)
...
```{r, LEFSE object for Environment}
ps.Env=subset_samples(ps.rncp, sample_data(ps.rncp)$Sample == "Environment" & sample_data(ps.rncp)$Date != "Blank") %>%
  prune_species(speciesSums(.) > 0, .)

sample_names(ps.Env) #make sure we have the samples we want

lefse.Env = as.data.frame(cbind(t(otu_table(ps.Env, taxa_are_rows = FALSE)), tax_table(ps.Env)))

lefse.Env$Info=paste(lefse.Env$Kingdom, lefse.Env$Phylum, lefse.Env$Class, lefse.Env$Order, lefse.Env$Family,lefse.Env$Genus, sep = "|")
lefse.Env$Kingdom = NULL
lefse.Env$Phylum=NULL
lefse.Env$Class=NULL
lefse.Env$Order=NULL
lefse.Env$Family=NULL
lefse.Env$Genus=NULL

LEFseMeta = as.data.frame(sample_data(ps.Env)) %>%
  select(., c(SampleType,Farm,SampleName)) %>%
  t(.) %>%
  as.data.frame(.) %>%
  mutate(., Info=row.names(.))

lefse.out=merge(lefse.Env, LEFseMeta, all=TRUE)
rm(lefse.Env, ps.Env)

write.csv(lefse.out, "~/PigLungs/lefseenv.csv", quote = FALSE)
```
...
```{r, attempt at SourceTracker for NCAT, not using this analysis}
#run
...
Rscript sourcetracker_for_qiime.r -i WCF_lavage_sourcetracker_ASV_table.txt -m WCF_lavage_sourcetracker_sample_data.txt -o lavage_full_WCF -v -n 25
```

```{r}
ps.WCF = subset_samples(ps.prok, sample_data(ps.prok)$Farm == "WCF" & sample_data(ps.prok)$SampleType != "Tissue" & sample_data(ps.prok)$SampleType != "Swab" & sample_data(ps.prok)$SampleName != "WCF_AIR_BLANK_S92")

write.dataset(ps.WCF, "~/PigLungs/sourcetracker/", "WCF_lavage_sourcetracker")

ps.WCF_Swab = subset_samples(ps.prok, sample_data(ps.prok)$Farm == "WCF" & sample_data(ps.prok)$SampleType != "Tissue" & sample_data(ps.prok)$SampleType != "Lavage" & sample_data(ps.prok)$SampleName != "WCF_AIR_BLANK_S92")

write.dataset(ps.WCF_Swab, "~/PigLungs/sourcetracker/", "WCF_swab_sourcetracker")

ps.WCF_Tissue = subset_samples(ps.prok, sample_data(ps.prok)$Farm == "WCF" & sample_data(ps.prok)$SampleType != "Swab" & sample_data(ps.prok)$SampleType != "Lavage" & sample_data(ps.prok)$SampleName != "WCF_AIR_BLANK_S92")

write.dataset(ps.WCF_Tissue, "~/PigLungs/sourcetracker/", "WCF_tissue_sourcetracker")


```

```{r, aggregation of sourcetracker txt files into dataframe for NCAT}
...
STDF_avg = bind_rows(swab_ST_avg, lavage_ST_avg, tissue_ST_avg) %>%
  #mutate(., Swab.Lavage = 0) %>%
  reshape2::melt(.)

STDF_avg$variable=factor(STDF_avg$variable,levels=c("Food","Water", "Soil", "Unknown", "Air"))

STDF_avg$Group=factor(STDF_avg$Group,levels=c("lavage","A_swab", "B_swab", "B_tissue", "A_tissue"))

g=ggplot() + geom_bar(aes(y = value, x = Group, fill = variable), data = STDF_avg, stat ="identity")+scale_fill_manual(values=c( "green4", "royalblue", "brown", "grey52", "deepskyblue3"))
...
g=ggplot(STDF_NCAT, aes(Group, value)) + facet_grid(. ~ variable) + stat_boxplot(fill = NA) #+ geom_jitter(width = 0.2, alpha = 0.5, aes(color = Group))+scale_fill_manual(values=c( "green4", "royalblue", "brown", "grey52", "deepskyblue3"))
```

```{r, WCF sourcetracker info}
...
STDF_WCF = bind_rows(swab_ST_all, lavage_ST_all, tissue_ST_all) %>%
  reshape2::melt(.)

STDF_WCF$variable=factor(STDF_WCF$variable,levels=c("Food","Water", "Soil", "Unknown", "Air"))

STDF_WCF$Group=factor(STDF_WCF$Group,levels=c("lavage","A_swab", "B_swab", "B_tissue", "A_tissue"))

ggplot(STDF_WCF, aes(Group, value)) + facet_grid(. ~ variable) + stat_boxplot(fill = NA) #+ geom_jitter(width = 0.2, alpha = 0.5)+scale_fill_manual(values=c( "green4", "royalblue", "brown", "grey52", "deepskyblue3"))

```

```{r, what happens when we remove air samples from SourceTracker analysis?, not used}
ps.WCF = subset_samples(ps.prok, sample_data(ps.prok)$Farm == "WCF" & sample_data(ps.prok)$SampleType != "Tissue" & sample_data(ps.prok)$SampleType != "Swab" & sample_data(ps.prok)$SampleName != "WCF_AIR_BLANK_S92" & sample_data(ps.prok)$SampleType != "Air")

sample_names(ps.WCF)

write.dataset(ps.WCF, "~/PigLungs/sourcetracker/", "WCF_lavage_noAir_sourcetracker")
```


```{r, aggregation of sourcetracker txt files into dataframe, swab/lavage analysis}
library(dplyr)
library(tidyr)
...
g=ggplot(SLDF, aes(variable, value)) + facet_grid(. ~ Group) + stat_boxplot(fill = NA) + ggtitle("SourceTracker") + labs(x = "Sample Type", y = "Relative Abundance")
g
...
ggplot() + geom_bar(aes(y = value, x = Group, fill = variable), data = SLDF2, stat ="identity")+scale_fill_manual(values=c( "green4", "royalblue", "brown", "grey52", "deepskyblue3", "chocolate"))
