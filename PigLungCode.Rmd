...
permanova = adonis(unifrac.dist ~ Farm*Sample, data = metadf)
...
require(ggplot2)
ps.NCAT = subset_samples(ps.rncp, sample_data(ps.rncp)$Farm == "NCAT") 

ordinate(ps.NCAT, method = "PCoA", distance = "unifrac") %>%
  plot_ordination(ps.NCAT, ., type="samples", color = "SampleType", shape = "Sample")

ps.WCF = subset_samples(ps.rncp, sample_data(ps.rncp)$Farm == "WCF")

ordinate(ps.WCF, method = "PCoA", distance = "unifrac") %>%
  plot_ordination(ps.WCF, ., type="samples", color = "SampleType", shape = "Sample")

NCATmeta = data.frame(sample_data(ps.NCAT))

unifrac.dist = UniFrac(ps.NCAT)

permanova = adonis(unifrac.dist ~ Sample*SampleSubtype, data = NCATmeta)
...
```{r, attempt at SourceTracker for NCAT}
#run
